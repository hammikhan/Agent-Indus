...
<script>
    $(document).ready(function() {
        var countFlights = {{ count($offerData['finaldata']['Flights']) }};
        ...
        $('input[type="radio"][name^="brand_ref_key"]').change(function() {
            ...
            // console.log(passengerFares);
            $.each(passengerFares, function(index, fare) {
                passBreak[index]['Quantity'] += parseInt(fare['Quantity']);

                passBreak[index]['BasePrice'] += parseInt(fare['BasePrice']) / countFlights;
                passBreak[index]['TaxPrice'] += parseInt(fare['Taxes']) / countFlights;
                passBreak[index]['TotalPaxPrice'] += parseInt(fare['TotalPrice']) / countFlights;
                if(fare['PaxType'] == 'Infant'){
                    passBreak[index]['Markup'] = 0;
                    passBreak[index]['Discount'] = 0;
                } else {
                    const discountValue = parseInt(fare['Discount']);
                    const markupValue = parseInt(fare['Markup']);
                    
                    passBreak[index]['Discount'] += isNaN(discountValue) ? 0 : discountValue * parseInt(fare['Quantity']);
                    passBreak[index]['Markup'] += isNaN(markupValue) ? 0 : markupValue * parseInt(fare['Quantity']);
                }

                const percentageValue = parseInt(fare['Percentage']);
                passBreak[index]['Percentage'] += isNaN(percentageValue) ? 0 : percentageValue;
            });
            
            if ($('input[name="brand_ref_key[1]"]:checked').length > 0) {
                var passengerfares2 = $('input[name="brand_ref_key[1]"]:checked').data('passengerfares');
                $.each(passengerfares2, function(index2, fare2) {
                    passBreak[index2]['BasePrice'] += parseInt(fare2['BasePrice']) / countFlights;
                    passBreak[index2]['TaxPrice'] += parseInt(fare2['Taxes']) / countFlights;
                    passBreak[index2]['TotalPaxPrice'] += parseInt(fare2['TotalPrice']) / countFlights;
                });
            }
            ...
            $.each(passBreak, function(index, paxfare) {
                ...
                TotalDiscount += paxfare.Discount;
                TotalMarkup += paxfare.Markup;
                SubTotal += paxfare.TotalPaxPrice;
                GrandTotal += TotalPerPax;
                
                if(paxfare.BasePrice != 0){
                    fareBreakdown += `
                        <tr>
                            <td>${paxfare.PaxType}</td>
                            <td>PKR ${formatCurrency(PaxBasePrice)}</td>
                            <td>PKR ${formatCurrency(paxfare.TaxPrice)}</td>
                            <td>${paxfare.Discount === 0 ? '+' : `-`} PKR ${paxfare.Markup !== 0 ? formatCurrency(paxfare.Markup) : (paxfare.Discount !== 0 ? formatCurrency(paxfare.Discount) : 0)} ${paxfare.Discount === 0 ? '' : `/ (${paxfare.Percentage}%)`}</td>
                            <td>${paxfare.Quantity}</td>
                            <td>PKR ${formatCurrency(paxfare.TotalPaxPrice)}</td>
                            <td class="fw-semibold">PKR ${formatCurrency(TotalPerPax)}</td>
                        </tr>
                        `;
                }
            });
            ...
            <tr>
                ...
                <td class="fw-bold text-secondary-light">${TotalDiscount === 0 ? '+' : `- `} PKR ${TotalMarkup !== 0 ? formatCurrency(TotalMarkup) : (TotalDiscount !== 0 ? formatCurrency(TotalDiscount) : 0)}</td>
            ...
            $('#billablePrice').text('PKR '+GrandTotal.toLocaleString());
